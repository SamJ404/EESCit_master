# Strongly followed by 'FUNSR'
# 'Keyan Chen, Continuous Remote Sensing Image Super-Resolution Based on Context Interaction in Implicit Function Space, IEEE TGRS, vol61, 4702216,2023'
# Modified by SAM J
...
from utils import calculate_psnr_pt
...
def  val(loader, model, crop_border=12):
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            pred = model(batch['img'], batch['gt'].shape[-2:])

        psnr = calculate_psnr_pt(pred, batch['gt'], crop_border=crop_border)
        psnr_avg += sum(psnr)

    psnr_avg = psnr_avg/len(pbar)
...
def main(config_, save_path, args):
    ...
    epoch_val_interval = config.get('epoch_val')
    epoch_save_interval = config.get('epoch_save')
    max_val_v = -1e18
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val_interval is not None) and (epoch % epoch_val_interval == 0):
            ...
            val_psnr = val(val_loader, model_, crop_border=config['val_dataset']['scale'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/train_symswin_EESCit_bestval.yaml')
    parser.add_argument('--n_gpus', default=1, type=int)
    parser.add_argument('--save_name', default='SymSwin_EESCitres_bestval')
