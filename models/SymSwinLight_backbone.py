...
class PSA(nn.Module):
    ...
    """
    def __init__(self, dim, window_size,permute_ratio, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):

        super().__init__()
        self.dim = dim
        self.window_size = window_size# Wh, Ww
        self.permuted_window_size = (window_size[0] // permute_ratio,window_size[1] // permute_ratio)
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((permute_ratio * self.permuted_window_size[0] - 1) * (permute_ratio * self.permuted_window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
        ...
        relative_coords[:, :, 0] *= permute_ratio * self.permuted_window_size[1] - 1
        ...
        aligned_relative_position_index = aligned_relative_position_index.reshape\
            (self.permuted_window_size[0],self.permuted_window_size[1],1,1,self.permuted_window_size[0]*self.permuted_window_size[1]).repeat(1,1,self.permute_ratio,self.permute_ratio,1)\
            .permute(0,2,1,3,4).reshape((permute_ratio**2)*self.permuted_window_size[0]*self.permuted_window_size[1],self.permuted_window_size[0]*self.permuted_window_size[1]) #  FN*FN,WN*WN
        ...
        self.kv = nn.Linear(dim, 2*dim//(self.permute_ratio**2), bias=qkv_bias)
    ...
    def forward(self, x, mask=None):
        ...
        kv = self.kv(x).reshape(b_, n//(self.permute_ratio**2), 2, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        ...
        if mask is not None:
            nw = mask.shape[0]
            attn = attn.view(b_ // nw, nw, self.num_heads, n, n//(self.permute_ratio**2)) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, n, n//(self.permute_ratio**2))
...
class PSA_Block(nn.Module):
    ...
    def __init__(self,
        ...
        self.permute_ratio = permute_ratio
        self.permuted_window_size = window_size//permute_ratio
    ...
    def calculate_mask(self, x_size):
        ...
        h, w = x_size
        permuted_window_mask = torch.zeros((1, h // self.permute_ratio, w // self.permute_ratio, 1))  # 1 h w 1
        h_slices = (slice(0, -self.permuted_window_size), slice(-self.permuted_window_size,
                                                            -self.shift_size // self.permute_ratio), slice(-self.shift_size // self.permute_ratio, None))
        w_slices = (slice(0, -self.permuted_window_size), slice(-self.permuted_window_size,
                                                            -self.shift_size // self.permute_ratio),
                    slice(-self.shift_size // self.permute_ratio, None))
        cnt = 0
        for h in h_slices:
            for w in w_slices:
    ...
    def forward(self, x, x_size):
        ...
        if self.input_resolution == [H_pad, W_pad]:
            attn_windows = self.attn(x_windows, mask=self.attn_mask)  # nw*b, window_size*window_size, c
        else:
            # attn_windows = self.attn(x_windows, mask=self.calculate_mask(x_size).to(x.device))
            attn_windows = self.attn(x_windows, mask=self.calculate_mask([H_pad, W_pad]).to(x.device))
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.csffa = CSFFA(dim=dim, norm_layer=norm_layer)
    ...
    def forward(self, x, x_size):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x = blk(x, x_size)
        x = self.csffa(shortcut, x, x_size)
...
@register('SymSwinLight_backbone')
class SRFormer_hw_csffa(nn.Module):
    r""" SRFormer
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(SRFormer_hw_csffa, self).__init__()
        num_in_ch = in_chans
        ...
        for i_layer in range(self.num_layers):
            layer = PSA_Group(
                ...
                permute_ratio=permute_ratio[i_layer],
